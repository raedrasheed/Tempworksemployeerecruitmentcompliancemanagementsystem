...
- PASS — 3. legacy Agency.isSystem-only user stamps true with fallback=true — agencyIsSystem=true
...
- PASS — 8. PlatformAdminAccessService reads Agency.isSystem only inside fallback branches — agencyReads=2 fallbackTags=2
- PASS — 9. runtime inventory: no direct Agency.isSystem authorization dependency outside allow-list — clean
- PASS — 10. Agency.isSystem column still exists in Prisma schema — present
