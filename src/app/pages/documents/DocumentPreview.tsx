...
export function DocumentPreview() {
  ...
  const handleVerify = async (action: 'VERIFY' | 'REJECT') => {
    setVerifying(true);
    try {
      const updated = await documentsApi.verify(id!, { action });
      setDocument(updated);
      toast.success(action === 'VERIFY' ? 'Document approved' : 'Document rejected');
    } catch (err: any) {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild>
          <Link to="/dashboard/documents-compliance"><ArrowLeft className="w-5 h-5" /></Link>
        </Button>
        <div className="flex-1">
          <h1 className="text-3xl font-semibold text-[#0F172A]">Document Preview</h1>
          <p className="text-muted-foreground mt-1">{document.documentType?.name} — {document.name}</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-4">
              ...
              {document.uploadedBy && (
                <div>
                  ...
                  <p className="font-medium mt-1">{document.uploadedBy.firstName} {document.uploadedBy.lastName}</p>
                </div>
              )}
              {document.verifiedBy && (
                <div>
                  <p className="text-sm text-muted-foreground">Verified By</p>
                  <p className="font-medium mt-1">{document.verifiedBy.firstName} {document.verifiedBy.lastName}</p>
                </div>
              )}
              {document.notes && (
                <div>
          ...
          {/* Verify / Reject — shown for PENDING documents to authorized users */}
