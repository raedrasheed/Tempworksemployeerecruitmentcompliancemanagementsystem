...
export function EmployeeDocumentExplorer() {
  ...
  const allSelectedAppDocs = selectedApplicants.flatMap(id => applicantDocuments[id] ?? []);
  ...
  const toggleAllApplicantDocs = () => {
    if (selectedApplicantDocs.length === allSelectedAppDocs.length) {
      setSelectedApplicantDocs([]);
    } else {
      setSelectedApplicantDocs(allSelectedAppDocs.map(d => d.id));
    }
  };
                  ...
                  {filteredApplicants.length === 0 ? (
                    ...
                    return (
                      <tr key={app.id} className={`border-b hover:bg-[#F8FAFC] transition-colors ${isSelected ? 'bg-[#F5F3FF]' : ''}`}>
                        ...
                        <td className="p-4">{app.nationality ?? '-'}</td>
                        <td className="p-4">
                          <Badge className={
                ...
                <table className="w-full">
                  <thead className="bg-[#F8FAFC] border-b">
                    <tr>
                      <th className="text-left p-4 w-12">
                        <Checkbox
                          checked={selectedApplicantDocs.length === allSelectedAppDocs.length && allSelectedAppDocs.length > 0}
                  ...
                  <tbody>
                    {allSelectedAppDocs.length === 0 ? (
                      <tr><td colSpan={7} className="p-8 text-center text-muted-foreground">No documents found for selected applicants</td></tr>
                    ) : allSelectedAppDocs.map(doc => {
